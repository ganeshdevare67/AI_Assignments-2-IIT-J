...
class GeneticAlgorithm:
    ...
    def evaluate_fitness(model):
        """
        Optimized fitness function: accuracy minus weighted penalty for conv and FC parameters.
        """
        # Existing accuracy calculation
        accuracy = model.evaluate_accuracy()

        # Optimized parameter penalty
        conv_params = sum([block.num_params() for block in model.conv_blocks])
        fc_params = sum([layer.num_params() for layer in model.fc_layers])

        conv_weight = 2.0  # Higher computational cost for conv layers
        fc_weight = 1.0    # Lower for FC layers
        scaling_factor = 10000  # Adjust as needed

        penalty = (conv_weight * conv_params + fc_weight * fc_params) / scaling_factor

        # Combine with any other existing penalties if needed
        fitness = accuracy - penalty

        # If you have other constraints/penalties, add them here
...
# Assignment 2 (Q1A)
    def selection(population, fitness_scores, num_selected):
        """
        Selects chromosomes using Roulette-Wheel selection based on relative fitness.
        Args:
            population (list): List of chromosomes.
            fitness_scores (list): List of fitness values for each chromosome.
